...
post_register_form.onsubmit = async (e) => {
  e.preventDefault();

  document.querySelector("#post_register_form button").disabled = true;
  document.querySelector(
    "#post_register_form button"
  ...
  console.log(post_register_form);

  const formData = new FormData(post_register_form);
  ...
  const registerResponse = await fetch(backendURL + "/api/service-provider", {
    method: "POST",
    headers: {
  ...
  // throw error
  if (!registerResponse.ok) throw new Error(await data.text());
  console.log(registerResponse);

  const data = registerResponse.json();

  if (registerResponse.ok) {
    post_register_form.reset();
    console.log(data);
    window.location.pathname = "/serviceprovider/serviceProviderDashboard.html";
  }
  document.querySelector("#post_register_form button").disabled = false;
  document.querySelector(
    "#post_register_form button"
